...
TorchAir cleans up static kernels when AOT samples exit. Fallback cleanup for legacy installations only accepts a validated `uninstall.sh` under the current CANN installation that matches a package generated by the current sample. Other paths in logs are not executed, and cleanup failures produce warnings.
