...
__aicore__ inline void auto_gen_te_superkernel_2_stream_4_ops_kernel_aic(void) {
    ...
    // sync all C->C kernel_name:is_inf__kernel0, send_info:{'is_inf__kernel0_2': 'cub:cub'}
    ...
    // Rule 1 : sync all aic must be insert behind each aic sub operator, when has real send info
    // sync all C->C kernel_name:is_inf__kernel0, send_info:{'pows__kernel0_1': 'cub:vec'}
    ffts_cross_core_sync(PIPE_FIX, AscendC::GetffstMsg(0x0, AscendC::SYNC_AIC_FLAG));
    wait_flag_dev(AscendC::SYNC_AIC_FLAG);

    // Rule 3.1 : sync all c2v must be insert when sendinfo has c2v, kernel_name:is_inf__kernel0, send_info:{'pows__kernel0_1': 'cub:vec'}
    // send sync of C->V;
    ffts_cross_core_sync(PIPE_MTE3, AscendC::GetffstMsg(0x02, AscendC::SYNC_AIC_AIV_FLAG));

    if ASCEND_IS_AIC {
        // kernel=is_inf__kernel0, ev=101, param_offset=13
        NotifyFunc<true>(param_base[13]);
    }
}
...
__aicore__ inline void auto_gen_te_superkernel_2_stream_4_ops_kernel_aiv(void) {
    ...
    // Rule 3.2 : sync all c2v must be insert when recvinfo has c2v, kernel_name:pows__kernel0, send_info:{'is_inf__kernel0_2': 'cub:vec'}
    // receive sync of C->V;
    ...
    // sync all V->V kernel_name:pows__kernel0, send_info:{'pows__kernel0_3': 'vec:vec'}
...
static const struct FunLevelMixCoreType te_superkernel_2_stream_4_ops_mix_aic_section __attribute__ ((used, section (".ascend.meta.te_superkernel_2_stream_4_ops_mix_aic"))) = { {{F_TYPE_KTYPE, sizeof(unsigned int)}, K_TYPE_MIX_AIC_MAIN},    {{F_TYPE_MIX_TASK_RATION, sizeof(unsigned int)}, 1, 1} };
#endif
#endif

#if TILING_KEY_VAR == 0UL
#if (defined(__DAV_VEC__) && __NPU_ARCH__ == 2201)
static const struct FunLevelMixCoreType te_superkernel_2_stream_4_ops_mix_aiv_section __attribute__ ((used, section (".ascend.meta.te_superkernel_2_stream_4_ops_mix_aiv"))) = { {{F_TYPE_KTYPE, sizeof(unsigned int)}, K_TYPE_MIX_AIC_MAIN},    {{F_TYPE_MIX_TASK_RATION, sizeof(unsigned int)}, 1, 1} };
#endif
#endif
#endif
