...
extern "C"  __global__ __attribute__((aligned(512))) __aicore__ void auto_gen_te_superkernel_2_stream_2_ops_kernel(void) {
    ...
    GM_ADDR workspace = param_base[14];
    ...
    #endif
    GM_ADDR profilingPtr = param_base[15];
    uint32_t taskId = *((__gm__ uint32_t*)(get_para_base() + 8 * 16));
    ...
    if ASCEND_IS_AIC {
      if (get_block_idx() < 32) {
        uint8_t coreid = (uint8_t)get_coreid();
        if ((coreid % 4) == 0) {
          is_finite__kernel0_middle(9);
        } else if ((coreid % 4) == 1) {
          is_finite__kernel0_middle_split1(9);
        } else if ((coreid % 4) == 2) {
          is_finite__kernel0_middle_split2(9);
        } else {
          is_finite__kernel0_middle_split3(9);
        }
