...
extern "C"  __global__ __attribute__((aligned(512))) __aicore__ void auto_gen_te_superkernel_2_kernel(void) {
    ...
    g_super_kernel_early_start_config = 5;
    if ASCEND_IS_AIV {
        if (true) {
            AscendC::WaitPreTaskEndImpl<5>();
        }
    }
    ...
    if ASCEND_IS_AIV {
      if (AscendC::GetBlockIdx() < 32) {
        uint8_t coreid = (uint8_t)get_coreid();
        if ((coreid % 4) == 0) {
          is_inf__kernel0_middle(8);
        } else if ((coreid % 4) == 1) {
          is_inf__kernel0_middle_split1(8);
        } else if ((coreid % 4) == 2) {
          is_inf__kernel0_middle_split2(8);
        } else {
          is_inf__kernel0_middle_split3(8);
        }
...
static const struct FunLevelMixCoreType te_superkernel_2_mix_aiv_section __attribute__ ((used, section (".ascend.meta.te_superkernel_2_mix_aiv"))) = { {{F_TYPE_KTYPE, sizeof(unsigned int)}, K_TYPE_MIX_AIV_MAIN},    {{F_TYPE_MIX_TASK_RATION, sizeof(unsigned int)}, 0, 1} };
#endif
#endif
