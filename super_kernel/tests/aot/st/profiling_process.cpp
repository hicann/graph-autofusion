...
int main(int argc, char **argv) {
    try {
        ...
        sk::test::SetProfilingPath(scenario == "empty_path" ? ""
                                                            : (std::filesystem::current_path() / "export").string());
        ...
        if (scenario == "memset_failure") {
            ...
            Require(logs.find("Failed to create output directory") != std::string::npos,
                    "directory failure not reached");
        } else if (enabled) {
