...
LAUNCH_CALLS = {
    "subprocess.Popen", "subprocess.run", "_run_argv",
    "multistream_runner._run_argv", "device_lease_runner.run_command",
}
KNOWN_CALLS = {
    ("superkernel-multistream-performance-tuning/scripts/multistream_runner.py", "_run_argv", "subprocess.Popen"): (False, "lease_primitive"),
    ("superkernel-multistream-performance-tuning/scripts/multistream_runner.py", "_run_phase", "_run_argv"): (True, "shared_device_leases"),
    ("superkernel-runtime-common/scripts/device_lease_runner.py", "run_command", "multistream_runner._run_argv"): (True, "shared_device_leases+lease_environment"),
    ("superkernel-multistream-performance-tuning/scripts/multistream_four_profile.py", "_run_profile", "multistream_runner._run_argv"): (True, "required_parent_marker"),
    ("superkernel-runtime-common/scripts/derived_family_lifecycle.py", "run_fresh_base", "device_lease_runner.run_command"): (True, "delegated_parent_wrapper"),
    ("superkernel-auto-tune/scripts/phase_entrypoint.py", "run", "device_lease_runner.run_command"): (True, "delegated_parent_wrapper"),
    ("superkernel-auto-tune/scripts/auto_tune_session.py", "run_agent_step", "subprocess.run"): (False, "agent_host_dispatch"),
    ("superkernel-runtime-common/scripts/recommend_sk_strategy.py", "_run_read_only_reanalysis", "subprocess.run"): (False, "offline_read_only_analysis"),
}
ENTRYPOINTS = (
    {
        "entrypoint": "parent_device_lease_runner",
        "path": "superkernel-runtime-common/scripts/device_lease_runner.py",
        "function": "run_command", "npu_capable": True,
    ...
    },
    {
        ...
        "function": "_run_phase", "npu_capable": True,
    ...
    },
    {
        ...
        "function": "run_plan", "npu_capable": True,
    ...
    },
    {
        ...
        "function": "run_fresh_base", "npu_capable": True,
...
def build_inventory(repository_root):
    ...
    for call in calls:
        ...
        if rule is None:
            ...
            classified.append({**call, "classification": "unregistered", "npu_capable": None})
            continue
        observed.add(key)
    ...
    for declaration in ENTRYPOINTS:
        ...
        entries.append({
            **{key: value for key, value in declaration.items() if key != "required_tokens"},
...
def validate_inventory(report, repository_root):
    ...
    if report != actual:
        raise ValueError("shared NPU lease inventory differs from deterministic repository scan")
    if report["status"] != "passed":
        raise ValueError("shared NPU lease inventory is blocked: " + "; ".join(report["blockers"]))
    return {
        "valid": True, "entrypoint_count": len(report["entrypoints"]),
...
def main():
    ...
    try:
        if args.validate:
            result = validate_inventory(json.loads(args.validate.read_text()), args.repository_root)
        else:
