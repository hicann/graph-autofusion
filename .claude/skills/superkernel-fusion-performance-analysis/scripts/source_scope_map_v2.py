...
def _validate_snapshot(root, provenance, source_manifest):
    ...
    expected = require_sha256(
        reference.get("manifest_sha256"), "provenance.source_snapshot_root.manifest_sha256"
    )
    if not manifest_path.is_file() or manifest_path.is_symlink() or file_sha256(manifest_path) != expected:
    ...
    observed = {}
    for index, item in enumerate(require_list(snapshot_manifest.get("files"), "snapshot files")):
        ...
        observed[safe_relative_path(item["relative_path"], "snapshot path").as_posix()] = path
    ...
    for index, unit in enumerate(require_list(source_manifest.get("units"), "source units")):
        ...
        import hashlib
        if hashlib.sha256(content[start:end]).hexdigest() != unit.get("source_span_sha256"):
            raise ValueError(f"source unit {index} byte span fingerprint mismatch")
        if canonical_sha256(
            require_object(unit.get("normalized_syntax"), f"source unit {index}.normalized_syntax")
        ...
        if source_manifest.get("revision_role") != "stable_source":
            for name, kind in (("begin", "scope_begin"), ("end", "scope_end")):
                marker = require_object(unit.get("marker_operations", {}).get(name), f"source unit {index} {name}")
                marker_start, marker_end = validate_byte_span(marker, len(content), f"source unit {index} {name}")
                if marker.get("operation_kind") != kind or not (start <= marker_start < marker_end <= end):
                    raise ValueError(f"source unit {index} {name} marker is invalid")
                if hashlib.sha256(content[marker_start:marker_end]).hexdigest() != marker.get("operation_sha256"):
                    raise ValueError(f"source unit {index} {name} marker fingerprint mismatch")
...
def _validate_dag(root, dag, references, projection, assignments, fused_inventory):
    ...
    for index, raw in enumerate(require_list(dag.get("nodes"), "provenance DAG nodes")):
        ...
        artifact_id = require_text(item.get("artifact_id"), f"DAG node {index}.artifact_id")
        if artifact_id in nodes:
            raise ValueError(f"duplicate provenance DAG artifact_id {artifact_id}")
        nodes[artifact_id] = item
    by_path = {
        require_text(item.get("relative_path"), f"provenance DAG {key}.relative_path"): item
        for key, item in nodes.items()
    }
    for artifact_id, item in nodes.items():
        artifact_path = resolve_evidence_path(
            root, item.get("relative_path"), f"provenance DAG {artifact_id}.relative_path"
        )
        expected_sha = require_sha256(
            item.get("sha256"), f"provenance DAG {artifact_id}.sha256"
        )
        if not artifact_path.is_file() or artifact_path.is_symlink() or file_sha256(artifact_path) != expected_sha:
            raise ValueError(f"provenance DAG artifact mismatch: {artifact_id}")
    for key, reference in references.items():
        path = reference.get("relative_path")
        if path not in by_path or by_path[path].get("sha256") != reference.get("sha256"):
            raise ValueError(f"provenance DAG does not seal {key}")
    dependencies = {}
    reverse = defaultdict(set)
    for artifact_id, item in nodes.items():
        deps = set(require_list(item.get("depends_on", []), f"DAG {artifact_id}.depends_on"))
    ...
    catalog = require_object(projection.get("evidence_catalog"), "projection.evidence_catalog")
    ...
    projection_evidence_paths = referenced_paths(
        [projection.get("evidence_catalog", {}), projection.get("runtime_validation", [])]
    )
    assignment_evidence_paths = referenced_paths(assignments.get("evidence_catalog", {}))
    ...
    if snapshot_path not in by_path or by_path[snapshot_path].get("sha256") != snapshot_reference.get("sha256"):
    ...
    for evidence_path in projection_evidence_paths:
        if not depends_transitively(projection_id, ids_by_path[evidence_path]):
            raise ValueError("projection evidence is not a dependency of calibration projection")
    ...
    for evidence_path in assignment_evidence_paths:
        if not depends_transitively(assignment_id, ids_by_path[evidence_path]):
            raise ValueError("assignment evidence is not a dependency of unit assignments")
    ...
    for evidence_path in fused_evidence_paths:
        if not depends_transitively(fused_id, ids_by_path[evidence_path]):
            raise ValueError("fused log evidence is not a dependency of fused inventory")
...
def _validate_normalized_record_evidence(
    ...
    catalog = require_object(artifact.get("evidence_catalog"), f"{label}.evidence_catalog")
    loaded_records = {}
    for artifact_id, reference in catalog.items():
        ...
        records = {}
        for index, raw in enumerate(require_list(evidence.get("records"), f"{label} evidence records")):
            record = require_object(raw, f"{label} evidence {artifact_id} record {index}")
            record_id = require_text(record.get("record_id"), f"{label} evidence record_id")
            if record_id in records:
                raise ValueError(f"{label} evidence {artifact_id} has duplicate record_id")
    ...
    for index, item in enumerate(items):
        ...
        for evidence_index, reference in enumerate(
            require_list(item.get("evidence_records"), f"{label}[{index}].evidence_records")
        ):
            reference = require_object(
                reference, f"{label}[{index}].evidence_records[{evidence_index}]"
            )
            artifact_id = require_text(reference.get("artifact_id"), "evidence artifact_id")
...
def _validate_current_source(source_root, source_manifest):
    ...
    for name, expected_sha in source_manifest["file_sha256"].items():
        ...
        if not source_file.is_file() or source_file.is_symlink() or file_sha256(source_file) != expected_sha:
            raise ValueError(f"current source differs from archived source snapshot: {name}")


def _snapshot_root(root, reference, label):
    reference = require_object(reference, label)
    relative = safe_relative_path(reference.get("relative_path"), f"{label}.relative_path")
    return resolve_evidence_path(root, relative.as_posix(), f"{label}.relative_path")


def load_source_scope_map_v2(path, *, expected_source_revision,
    ...
    if provenance.get("source_scope_map_content_fingerprint") != _content_fingerprint(value):
    ...
    if source_revision != require_text(expected_source_revision, "expected source revision"):
        raise ValueError("source_scope_map_v2 source revision differs from --source-revision")
    if source_revision_role == "stable_marker":
        ...
        if provenance.get("stable_marker_revision") is not None:
            raise ValueError("stable_source source maps must not declare stable_marker_revision")
        missing_auto = sorted(
            {
                "calibration_source_snapshot_root",
                *AUTO_SOURCE_ARTIFACT_KEYS,
    ...
    if projection.get("original_collection_fingerprint") != provenance[
        "original_candidate_collection_manifest"
    ]["fingerprint"]:
        raise ValueError("calibration projection original collection fingerprint mismatch")
    if projection.get("calibration_collection_fingerprint") != provenance[
        "calibration_collection_manifest"
    ]["fingerprint"]:
        raise ValueError("calibration projection calibration collection fingerprint mismatch")
    ...
    if expected_candidate_manifest_sha256 is not None:
        expected = require_sha256(
            expected_candidate_manifest_sha256, "expected candidate collection manifest SHA-256"
        )
        if references["original_candidate_collection_manifest"]["sha256"] != expected:
            raise ValueError("source map is not bound to the current candidate collection manifest")
    if expected_baseline_manifest_sha256 is not None:
        expected = require_sha256(
            expected_baseline_manifest_sha256, "expected baseline collection manifest SHA-256"
        )
        if references["baseline_collection_manifest"]["sha256"] != expected:
            raise ValueError("source map is not bound to the current baseline collection manifest")
    mapping_by_fingerprint = {
        item["sk_occurrence_fingerprint"]: item for item in sk_map["mappings"]
    }
    lookup = {}
    for index, raw in enumerate(require_list(value.get("source_ranges"), "source_ranges")):
        item = validate_exact_keys(
            raw,
            {
                "device_id", "model_id", "block_instance_id", "structural_family",
                "source_scope", "candidate_source_scope", "unit_id", "sk_occurrence_fingerprint",
                "baseline_projection_fingerprint", "relation", "boundary",
        ...
        fingerprint = require_sha256(
            item.get("sk_occurrence_fingerprint"), f"source_ranges[{index}].sk_occurrence_fingerprint"
        )
        ...
        if mapping.get("confidence") not in {"source_unit_exact", "source_unit_template_exact"}:
            raise ValueError(f"source range {index} lacks source-unit exact confidence")
        if len(mapping.get("covered_unit_ids", [])) != 1 or item.get("unit_id") != mapping["covered_unit_ids"][0]:
        ...
        expected_ops = [occurrence[2] for occurrence in mapping["original_child_occurrences"]]
        if item.get("ordered_child_op_sequence") != expected_ops:
            raise ValueError(f"source range {index} child op sequence mismatch")
        if item.get("baseline_projection_fingerprint") != mapping.get("baseline_projection_fingerprint"):
            raise ValueError(f"source range {index} baseline projection fingerprint mismatch")
        boundary = validate_exact_keys(
            item.get("boundary"),
            {"start_op", "end_op", "source_file", "start_offset", "end_offset"},
            f"source_ranges[{index}].boundary",
            required=("start_op", "end_op", "source_file", "start_offset", "end_offset"),
        )
        interval = mapping["source_intervals"][0]
        if any(boundary.get(field) != interval.get(field) for field in ("source_file", "start_offset", "end_offset")):
            raise ValueError(f"source range {index} boundary differs from source manifest")
        if (
...
def build_source_scope_map_v2(*, artifact_root, stable_marker_revision=None,
                              stable_source_revision=None,
                              provenance_dag, source_manifest, source_snapshot_root,
                              sk_source_map, block_instance_inventory,
                              calibration_projection, original_fused_inventory,
                              unit_assignments, original_candidate_collection_manifest,
                              calibration_collection_manifest,
                              baseline_collection_manifest,
    ...
    paths = {
        ...
        "original_candidate_collection_manifest": Path(original_candidate_collection_manifest),
    ...
    provenance = {
        **{key: artifact_reference(path, relative_to=root) for key, path in paths.items()},
    }
    if source_revision_role == "stable_source":
        provenance.update(
            {
    ...
    for mapping in sk_map["mappings"]:
        if mapping["relation"] != "exact_cover" or mapping["confidence"] not in {
            "source_unit_exact", "source_unit_template_exact"
        }:
            continue
        ...
        source_ranges.append(
            {
                ...
                "baseline_projection_fingerprint": mapping["baseline_projection_fingerprint"],
                ...
                "ordered_child_op_sequence": [item[2] for item in mapping["original_child_occurrences"]],
            }
        )
    value = {
        "schema_version": (
            "2.1" if source_revision_role == "stable_source" else "2.0"
        ),
        "protocol": "source_scope_map_v2",
        "provenance": provenance,
        "source_ranges": sorted(source_ranges, key=lambda item: item["sk_occurrence_fingerprint"]),
    }
