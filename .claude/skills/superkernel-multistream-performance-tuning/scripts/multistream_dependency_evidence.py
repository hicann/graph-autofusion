...
def _file_record(root, value, label):
    ...
    if value["size_bytes"] != path.stat().st_size or value["file_fingerprint"] != actual:
...
def _provider(root, value, label, statement_ids):
    required = {
        "provider_id", "provider_api_version", "provider_kind", "implementation",
        "input_files", "covered_dependency_kinds", "edges", "blockers",
    }
    ...
    implementation = _file_record(root, value["implementation"], f"{label}.implementation")
    inputs = value["input_files"]
    if not isinstance(inputs, list) or not inputs:
        raise ValueError(f"{label}.input_files must be non-empty")
    input_files = [_file_record(root, item, f"{label}.input_files[{index}]") for index, item in enumerate(inputs)]
    ...
    coverage = [_text(item, f"{label}.covered_dependency_kinds[]").upper() for item in coverage]
    ...
    for index, edge in enumerate(edges):
        ...
        if not isinstance(edge, dict) or set(edge) != {
            "before", "after", "kind", "evidence_locator"
        }:
        ...
        normalized_edges.append({
            ...
            "after": after,
            "kind": kind,
            "evidence_locator": _text(edge["evidence_locator"], f"{edge_label}.evidence_locator"),
        })
    blockers = value["blockers"]
    if not isinstance(blockers, list) or any(
        not isinstance(item, str) or not item.strip() or item != item.strip() for item in blockers
    ):
    ...
    return {
        ...
        "edges": sorted(normalized_edges, key=lambda item: (item["before"], item["after"], item["kind"])),
...
def build(fragment_set_path, artifact_root=None):
    ...
    required = {
        "schema_version", "fragment_set_id", "request_fingerprint", "range_id",
        "graph_occurrence_fingerprint", "statement_ids", "required_dependency_kinds",
        "providers", "fragment_set_fingerprint",
    }
    if not isinstance(value, dict) or set(value) != required:
        raise ValueError(f"dependency fragment set must contain exactly {sorted(required)}")
    if value["schema_version"] != FRAGMENT_SET_SCHEMA:
        raise ValueError(f"dependency fragment set must use {FRAGMENT_SET_SCHEMA}")
    unsigned = {key: item for key, item in value.items() if key != "fragment_set_fingerprint"}
    ...
    if not isinstance(required_kinds, list) or set(required_kinds) != HARD_DEPENDENCY_KINDS:
        raise ValueError("required_dependency_kinds must contain every hard dependency kind")
    ...
    if _has_cycle(statement_ids, {(item["before"], item["after"]) for item in hard_dependencies}):
    ...
    evidence = {
        ...
        "coverage": {kind: sorted(coverage.get(kind, [])) for kind in sorted(HARD_DEPENDENCY_KINDS)},
        ...
        "edge_sources": [
            {
                ...
                "after": key[1],
                "kind": key[2],
                "sources": sorted(edge_sources[key], key=lambda item: (item["provider_id"], item["evidence_locator"])),
            }
            for key in sorted(edge_sources)
        ],
...
def validate(
    evidence_path, artifact_root=None, *, require_complete=False,
    request_fingerprint=None, range_id=None, graph_occurrence_fingerprint=None,
    statement_ids=None,
):
    ...
    unsigned = {key: item for key, item in value.items() if key != "evidence_fingerprint"}
...
def main(argv=None):
    ...
    try:
        if args.command == "build":
            ...
            result = validate(
                args.evidence, args.artifact_root, require_complete=args.require_complete
            )
