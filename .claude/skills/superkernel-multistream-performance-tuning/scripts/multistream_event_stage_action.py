...
def _validate_analysis(value):
    fields = {
        "schema_version", "capture_fingerprint", "request_fingerprint", "decision",
        "targets", "analysis_fingerprint",
    }
...
def build(analysis, graph):
    ...
    if analysis["request_fingerprint"] != graph["request_fingerprint"]:
        raise ValueError("critical path analysis and logical graph request fingerprints differ")
    ...
    for target in sorted(analysis["targets"], key=lambda item: item["join_id"]):
        ...
        for event_id in sorted(target.get("actionable_event_edge_ids", [])):
            ...
            if event is None or event["producer_stage_id"] != stage_id:
                raise ValueError("analysis event action does not bind the critical producer stage")
            ...
            actions.append({
                ...
                "expected_dispatch_change": {
                    "kind": "event_notify_earlier", "logical_id": event_id,
                },
        ...
        for stage_id in sorted(target.get("actionable_stage_ids", [])):
            ...
            if stage is None or stage["movable"] is not True:
                raise ValueError("analysis action references non-movable or unknown stage")
            ...
            actions.append({
                ...
                "event_edge_id": (
                    ...
                    if target.get("actionable_event_edge_ids") else None
                ),
                "risk": "medium",
                "activation_condition": (
                    "event_clean3_nonregressing_without_incremental_gain"
                    if event_action_id else "immediate"
                ),
                "parent_action_id": event_action_id,
                "expected_dispatch_change": {
                    "kind": "stage_dispatch_earlier", "logical_id": stage_id,
                },
                "safety_requirements": list(STAGE_REQUIREMENTS),
            })
    actions.sort(key=lambda item: (item["activation_condition"] != "immediate", item["risk"], item["action_id"]))
    result = {
...
def validate(value):
    fields = {"schema_version", "request_fingerprint", "critical_path_analysis_fingerprint", "actions", "action_catalog_fingerprint"}
    ...
    unsigned = {key: item for key, item in value.items() if key != "action_catalog_fingerprint"}
    ...
    for index, action in enumerate(value["actions"]):
        required = {
            "action_id", "change_kind", "join_id", "stage_id", "event_edge_id", "risk",
            "activation_condition", "parent_action_id", "expected_dispatch_change",
            "safety_requirements",
        }
        ...
        requirements = EVENT_REQUIREMENTS if action["change_kind"] == "event_edge_refinement" else STAGE_REQUIREMENTS
        ...
        expected_id = action["event_edge_id"] if action["change_kind"] == "event_edge_refinement" else action["stage_id"]
        ...
        if parent is not None:
            parents.add((action_id, _text(parent, f"actions[{index}].parent_action_id")))
...
def main(argv=None):
    ...
    try:
        if args.command == "build":
            ...
            result = build(json.loads(args.analysis.read_text()), json.loads(args.graph.read_text()))
