...
DEVICE_PHASE_STATES = {
    "correctness_passed", "profile_collected", "clean3_passed", "clean5_passed"
}
...
def validate_plan(plan, require_fingerprint=True):
    ...
    expected_keys = {
        "schema_version", "trial_id", "request_fingerprint", "environment",
        "workspace_root", "artifact_root", "lease_root", "device_ids",
        "lease_timeout_seconds", "phases",
    }
    if require_fingerprint:
        expected_keys.add("plan_fingerprint")
    optional_keys = {"pre_profile_evidence"}
    if not expected_keys.issubset(set(plan)) or set(plan) - expected_keys - optional_keys:
    ...
    if not isinstance(phases, list) or [item.get("state_after") for item in phases if isinstance(item, dict)] != list(PHASE_STATES):
        raise ValueError(f"execution plan phases must exactly follow {list(PHASE_STATES)}")
    ...
    if pre_profile is not None:
        ...
        if pre_profile.get("kind") not in {
            "dispatch_order", "component_dispatch_order", "event_stage_dispatch"
        }:
            raise ValueError(
        ...
        normalized_pre_profile = {
            ...
            "path": _safe_relative(pre_profile.get("path"), "pre_profile_evidence.path"),
        }
    seen_paths = set()
    seen_phase_ids = set()
    normalized_phases = []
    for index, phase in enumerate(phases):
        required_keys = {
            "phase_id", "state_after", "argv", "validator_argv", "cwd",
            "timeout_seconds", "validator_timeout_seconds", "requires_device",
            "environment_overrides", "validator_exit_actions",
            "program_files", "required_artifacts", "manifest",
        }
        if set(phase) != required_keys:
            raise ValueError(f"phases[{index}] must contain exactly {sorted(required_keys)}")
        if not isinstance(phase["phase_id"], str) or not IDENTIFIER.fullmatch(phase["phase_id"]):
        ...
        if phase["requires_device"] and not device_ids:
            raise ValueError(f"phases[{index}] requires a device but device_ids is empty")
        ...
        for exit_code, action in exit_actions.items():
            if not isinstance(exit_code, str) or not exit_code.isdigit() or not 0 <= int(exit_code) <= 255:
                raise ValueError(f"phases[{index}] has an invalid validator exit code")
            if action not in VALIDATOR_EXIT_ACTIONS:
                raise ValueError(f"phases[{index}] has an invalid validator exit action")
            if action == "pass" and exit_code != "0":
                raise ValueError(f"phases[{index}] only exit code 0 may map to pass")
        reject_codes = [code for code, action in exit_actions.items() if action == "reject"]
        if phase["state_after"] in {"clean3_passed", "clean5_passed"} and not reject_codes:
            raise ValueError(f"phases[{index}] clean validator must define a reject exit code")
        if phase["state_after"] not in {"clean3_passed", "clean5_passed"} and reject_codes:
        ...
        for program_index, program in enumerate(program_files):
            if not isinstance(program, dict) or set(program) != {"path", "file_fingerprint"}:
                raise ValueError(
                    f"phases[{index}].program_files[{program_index}] must contain path and file_fingerprint"
                )
            path = Path(program["path"])
            if not path.is_absolute() or not path.is_file():
                raise ValueError(f"phases[{index}] program file must be an existing absolute file")
            resolved = str(path.resolve())
            if resolved in seen_programs:
                raise ValueError(f"phases[{index}] has duplicate program file: {resolved}")
            seen_programs.add(resolved)
            actual = file_fingerprint(resolved)
            if program["file_fingerprint"] != actual:
                raise ValueError(f"phases[{index}] program file fingerprint mismatch: {resolved}")
        ...
        for artifact_index, artifact in enumerate(artifacts):
            relative = _safe_relative(artifact, f"phases[{index}].required_artifacts[{artifact_index}]")
        ...
        normalized_phase = {
            **phase,
            ...
            "validator_argv": _validate_argv(phase["validator_argv"], f"phases[{index}].validator_argv"),
            "cwd": _safe_relative(phase["cwd"], f"phases[{index}].cwd"),
            "environment_overrides": _validate_environment(phase["environment_overrides"]),
            "validator_exit_actions": dict(sorted(exit_actions.items(), key=lambda item: int(item[0]))),
            "program_files": sorted(normalized_programs, key=lambda item: item["path"]),
            "required_artifacts": normalized_artifacts,
            "manifest": manifest,
        }
        for executable in (
            normalized_phase["argv"][0], normalized_phase["validator_argv"][0]
        ):
        ...
        for log_path in _phase_log_paths(normalized_phase).values():
            if log_path in seen_paths:
                raise ValueError(f"derived phase log path collides with another output: {log_path}")
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        handle.close()
                        raise TimeoutError(f"timed out acquiring NPU device lease {device_id}")
                    time.sleep(0.05)
            handle.seek(0)
            handle.truncate()
            handle.write(json.dumps({"pid": os.getpid(), "device_id": device_id, "acquired_at": _now()}))
            handle.flush()
...
def _sealed_file(path, relative):
    return {"path": relative, "size_bytes": path.stat().st_size, "fingerprint": file_fingerprint(path)}
...
def validate_phase_manifest(manifest, plan, phase, artifact_root, require_passed=True):
    ...
    if manifest.get("status") == "passed":
        expected_keys = {
            "schema_version", "trial_id", "request_fingerprint", "plan_fingerprint",
            "phase_id", "state_after", "status", "reason", "cwd",
            "workspace_root", "artifact_root", "lease_root", "environment",
            "device_ids", "command", "validator", "sealed_files", "completed_at",
        }
    ...
    if manifest.get("cwd") != phase["cwd"] or manifest.get("environment") != _phase_environment(plan, phase):
    ...
    for key, argv in (("command", phase["argv"]), ("validator", phase["validator_argv"])):
        record = manifest.get(key)
        if not isinstance(record, dict) or record.get("argv") != argv:
            raise ValueError(f"phase manifest {key} argv mismatch")
        if manifest.get("status") == "passed":
            expected_record = {
                "argv", "pid", "started_at", "finished_at", "return_code", "timed_out"
            }
            if set(record) != expected_record:
                raise ValueError(f"passed phase {key} record is incomplete")
            if not isinstance(record.get("pid"), int) or isinstance(record.get("pid"), bool) or record["pid"] <= 0:
    ...
    if manifest.get("status") == "passed" and (
        ...
        raise ValueError("passed phase sealed_files must exactly match declared outputs")
    for relative in required:
        ...
        if item is None or item.get("fingerprint") != file_fingerprint(path) or item.get("size_bytes") != path.stat().st_size:
...
def _write_terminal_manifest(plan, phase, artifact_root, status, reason, records):
    ...
    manifest = {
        ...
        "command": records.get("command", {"argv": phase["argv"], "return_code": None, "timed_out": False}),
        "validator": records.get("validator", {"argv": phase["validator_argv"], "return_code": None, "timed_out": False}),
        "sealed_files": sealed,
        "completed_at": _now(),
    }
...
def _run_phase(plan, phase, workspace_root, artifact_root, lease_root):
    ...
    logs = _phase_log_paths(phase)
    output_paths = [_under(artifact_root, value, "phase log") for value in logs.values()]
    artifact_paths = [_under(artifact_root, value, "required artifact") for value in phase["required_artifacts"]]
    existing = [str(path) for path in output_paths + artifact_paths if path.exists()]
    if existing:
        raise ValueError(f"phase has pre-existing outputs without a manifest: {existing}")
    ...
    try:
        with shared_device_leases(
            lease_ids, lease_root, plan["lease_timeout_seconds"]
        ):
            records["command"] = _run_argv(
                phase["argv"], cwd, environment,
            ...
            if records["command"]["timed_out"] or records["command"]["return_code"] != 0:
                reason = "command_timeout" if records["command"]["timed_out"] else "command_failed"
                return _write_terminal_manifest(plan, phase, artifact_root, "failed", reason, records)
            records["validator"] = _run_argv(
                phase["validator_argv"], cwd, environment,
            ...
            if records["validator"]["timed_out"] or records["validator"]["return_code"] != 0:
                if records["validator"]["timed_out"]:
                    return _write_terminal_manifest(
                        plan, phase, artifact_root, "failed", "validator_timeout", records
                    )
                exit_code = str(records["validator"]["return_code"])
                action = phase["validator_exit_actions"].get(exit_code, "fail")
                status = {"reject": "rejected", "block": "blocked", "fail": "failed"}[action]
                return _write_terminal_manifest(
                    plan,
                    phase,
                    artifact_root,
        ...
        return _write_terminal_manifest(plan, phase, artifact_root, "blocked", str(error), records)
    except OSError as error:
        return _write_terminal_manifest(
            plan, phase, artifact_root, "failed", f"process_start_failed: {error}", records
        )
    missing = [relative for relative, path in zip(phase["required_artifacts"], artifact_paths) if not path.is_file()]
    if missing:
        return _write_terminal_manifest(plan, phase, artifact_root, "failed", f"required artifacts missing: {missing}", records)
    sealed = [_sealed_file(path, relative) for relative, path in zip(phase["required_artifacts"], artifact_paths)]
    for relative in logs.values():
        sealed.append(_sealed_file(_under(artifact_root, relative, "phase log"), relative))
    manifest = {
        "schema_version": PHASE_SCHEMA,
        "trial_id": plan["trial_id"],
        "request_fingerprint": plan["request_fingerprint"],
...
def run_plan(plan, state_path, workspace_root, artifact_root, lease_root):
    ...
    state_indices = {name: index for index, name in enumerate(multistream_execution.ORDERED_STATES)}
    if state["state"] not in state_indices or state_indices[state["state"]] < state_indices["diff_verified"]:
    ...
    for phase in plan["phases"]:
        ...
        if current_index >= phase_index:
            if not manifest_path.is_file():
                raise ValueError(f"advanced state lacks phase manifest: {phase['manifest']}")
            validate_phase_manifest(json.loads(manifest_path.read_text()), plan, phase, artifact_root)
            continue
        if phase_index != current_index + 1:
            raise ValueError(f"state cannot resume at phase {phase['phase_id']}")
        if phase["state_after"] == "profile_collected" and plan.get("pre_profile_evidence"):
            ...
            if not evidence_path.is_file():
                if kind in {"dispatch_order", "component_dispatch_order"}:
                    raise ValueError("reorder profile is blocked until dispatch_order_evidence exists")
                raise ValueError("profile is blocked until event_stage_dispatch evidence exists")
            if kind == "dispatch_order":
                multistream_operator_order.validate_dispatch_evidence(
                    evidence_path, artifact_root, trial_id=plan["trial_id"],
                    request_fingerprint=plan["request_fingerprint"],
                )
                ...
                evidence = multistream_event_stage_dispatch.validate(
                    json.loads(evidence_path.read_text()), action,
                    trial_id=plan["trial_id"],
                    request_fingerprint=plan["request_fingerprint"],
                )
                if evidence["decision"] != "effective":
                    raise ValueError("profile is blocked because event/stage dispatch action is not effective")
        if manifest_path.is_file():
            manifest = json.loads(manifest_path.read_text())
            validate_phase_manifest(manifest, plan, phase, artifact_root)
        else:
            manifest = _run_phase(plan, phase, workspace_root, artifact_root, lease_root)
        if manifest["status"] != "passed":
            terminal = {
                "blocked": "blocked", "rejected": "rejected", "failed": "failed"
            }[manifest["status"]]
            state = multistream_execution.advance_state(state, terminal, phase["manifest"])
            _atomic_write_json(state_path, state)
            raise RuntimeError(f"phase {phase['phase_id']} {manifest['status']}: {manifest['reason']}")
        state = multistream_execution.advance_state(state, phase["state_after"], phase["manifest"])
        _atomic_write_json(state_path, state)
    return {"valid": True, "state": state["state"], "plan_fingerprint": plan["plan_fingerprint"]}
...
def main(argv=None):
    ...
    try:
        plan = json.loads((args.draft if args.command == "freeze-plan" else args.plan).read_text())
        if args.command == "freeze-plan":
            ...
            result = run_plan(plan, args.state, args.workspace_root, args.artifact_root, args.lease_root)
