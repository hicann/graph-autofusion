...
class JoinValidationTest(unittest.TestCase):
    ...
    def test_rejects_candidate_when_join_stall_grows(self):
        ...
        result = multistream_join_validation.compare("trial-1", self.ACTION_FP, baseline, candidate)
    ...
    def test_rejects_misaligned_occurrences(self):
        ...
        candidate["analysis_fingerprint"] = multistream_critical_path.fingerprint(
            {key: value for key, value in candidate.items() if key != "analysis_fingerprint"}
        )
        with self.assertRaisesRegex(ValueError, "alignment"):
            multistream_join_validation.compare("trial-1", self.ACTION_FP, baseline, candidate)
