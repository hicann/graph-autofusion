...
UNINSTALL_SHELL="${curpath}/run_graph_autofusion_uninstall.sh" # 卸载脚本路径
...
log_operation() {
    ...
    echo "${RUN_CMD_TYPE} ${LEVEL} ${RUN_USERNAME} ${cur_date} ${HOST} graph_autofusion $2 installmode=${RUN_CMD}; cmdlist=--${RUN_CMD}" >> "${OPERATION_LOG_FILE}"
}
...
user_auth() {
    ...
    if [ "${run_user_id}" -ne 0 ]; then
        if [ "${run_user_id}" -ne "${dir_user_id}" ]; then
            err_no0x0093 "Current user is not supported to ${RUN_CMD} the graph_autofusion package"
        fi
    fi
}
...
uninstall_run() {
    ...
    local graph_autofusion_install_path_param="$(get_pkg_install_path_param)"
    local install_top_path="$(get_install_top_path)"

    if [ -f "$ASCEND_INSTALL_INFO_FILE" ]; then
        local graph_autofusion_install_type=$(get_install_param "GRAPH_AUTOFUSION_Install_Type" "${ASCEND_INSTALL_INFO_FILE}")
    elif [ -f "${ASCEND_INSTALL_INFO_OLD_FILE}" ]; then
        num=$(grep -c -i graph_autofusion_install_path_param "${ASCEND_INSTALL_INFO_OLD_FILE}")
        if [ "${num}" != "0" ]; then
            local graph_autofusion_install_type="$(grep -iw graph_autofusion_install_type "$ASCEND_INSTALL_INFO_OLD_FILE" | cut -d"=" -f2-)"
        fi
    else
        err_no0x0080 "please complete ${ASCEND_INSTALL_INFO_FILE} or ${ASCEND_INSTALL_INFO_OLD_FILE}"
    fi
    if [ $? -eq 0 ]; then
        log "INFO" "${RUN_CMD} ${graph_autofusion_install_path_param} ${graph_autofusion_install_type}"
        bash "${UNINSTALL_SHELL}" "${RUN_CMD}" "${graph_autofusion_install_path_param}" "${graph_autofusion_install_type}" "${IS_QUIET}" "n" "" "y" "$version_dir"
        if [ $? -eq 0 ]; then
            ...
            if [ $? -eq 0 ] && [ -f "${ASCEND_INSTALL_INFO_OLD_FILE}" ] && [ -w "${ASCEND_INSTALL_INFO_OLD_FILE}" ] && [ "${num}" != "0" ]; then
                sed -i '/graph_autofusion_install_path_param=/Id' "${ASCEND_INSTALL_INFO_OLD_FILE}"
                sed -i '/graph_autofusion_install_type=/Id' "${ASCEND_INSTALL_INFO_OLD_FILE}"
            fi
