...
extern "C" __global__ __aicore__ void i0_store_test(GM_ADDR x1, GM_ADDR y1, GM_ADDR workspace, GM_ADDR tiling);
...
class E2EBackendI0StoreCode : public testing::Test, public testing::WithParamInterface<std::vector<int>> {};

static float I0Reference(float x) {
...
TEST_P(E2EBackendI0StoreCode, CalculateCorrect) {
  ...
  ICPU_RUN_KF(i0_store_test, tiling_data.block_dim, reinterpret_cast<uint8_t *>(x), reinterpret_cast<uint8_t *>(y),
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2EBackendI0StoreCode,
