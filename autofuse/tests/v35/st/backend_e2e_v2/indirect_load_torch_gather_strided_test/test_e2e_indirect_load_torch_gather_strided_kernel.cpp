...
void Initialize(float *data, int64_t *index, std::vector<float> &expected) {
  ...
  for (int32_t a0 = 0; a0 < 8; ++a0) {
    for (int32_t a1 = 0; a1 < 16; ++a1) {
      for (int32_t a2 = 0; a2 < 5; ++a2) {
        const int32_t output_offset = 80 * a0 + 5 * a1 + a2;
        ...
        expected[static_cast<size_t>(output_offset)] =
            data[kInputStride0 * a0 + kInputStride1 * index_value + kInputStride2 * a2];
      }
    }
  }
}
