...
#define GET_TILING_DATA(t, tiling)  AutofuseTilingData t = *(AutofuseTilingData*)tiling;
  ...
  GenerateBackendKernelWithCheck(graph, shape_info, tiling_stub, [](const std::string &kernel) {
    EXPECT_NE(kernel.find("CompareExtend<float, 2, CMPMODE::GE>"), std::string::npos);
    EXPECT_NE(kernel.find("{static_cast<uint16_t>(z1t_actual_size), static_cast<uint16_t>(z2t_actual_size)}"),
              std::string::npos);
