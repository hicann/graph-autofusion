...
namespace {
void BuildWhereScalarCvStageGraph(af::AscGraph &graph, const af::Expression &s0, const af::Expression &s1,
...
void InitWhereLoadAttrs(const af::AscNodePtr &load, const af::Expression &stride0, int64_t tensor_id,
                        const af::Axis &z0, const af::Axis &z1, const af::Axis &z2) {
  ...
  load->outputs[0].attr.vectorized_strides = {stride0, One};
...
void InitWhereOutputAttrs(const af::AscNodePtr &where, const af::Expression &s2, const af::Axis &z0, const af::Axis &z1,
                          const af::Axis &z2) {
  where->attr.api.compute_type = af::ComputeType::kComputeElewise;
  where->attr.api.type = af::ApiType::kAPITypeCompute;
  where->attr.api.unit = af::ComputeUnit::kUnitVector;
...
void InitWhereScalarCvStageAttrs(af::AscGraph &graph, const af::Expression &s2, const af::Axis &z0, const af::Axis &z1,
                                 const af::Axis &z2) {
  InitWhereLoadAttrs(graph.FindNode("load1"), s2, 0, z0, z1, z2);
  InitWhereLoadAttrs(graph.FindNode("load2"), s2 + s2, 1, z0, z1, z2);
  InitWhereLoadAttrs(graph.FindNode("load3"), s2 + s2, 2, z0, z1, z2);
  InitWhereOutputAttrs(graph.FindNode("where"), s2, z0, z1, z2);
}

void InitWhereScalarCvStageTpipe(codegen::TPipe &tpipe, codegen::Tiler &tiler, const af::AscGraph &graph,
                                 const af::AscNodePtr &load1, const af::AscNodePtr &load2, const af::AscNodePtr &load3,
                                 const af::AscNodePtr &where, const af::Expression &s0, const af::Expression &s1,
...
TEST(WhereRegApiCallTest, WhereRegApiCall_Scalar_x2x3_CVStage) {
  af::AscGraph graph("test_graph");
  ...
  auto z1 = graph.CreateAxis("z1", s1);
  auto z2 = graph.CreateAxis("z2", s2);
  BuildWhereScalarCvStageGraph(graph, s0, s1, s2, z0, z1, z2);
  InitWhereScalarCvStageAttrs(graph, s2, z0, z1, z2);
  ...
  codegen::Tiler tiler;
  codegen::TPipe tpipe("tpipe", tiler);
  InitWhereScalarCvStageTpipe(tpipe, tiler, graph, load1, load2, load3, where, s0, s1, s2, z0, z1, z2);
  std::vector<af::AxisId> current_axis;
  current_axis.push_back(z0.id);
