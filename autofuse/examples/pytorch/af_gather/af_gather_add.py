...
class MyModel(nn.Module):
    ...
    def forward(self, x, indices, y):
        result = torch.add(torch.gather(x, 1, indices), y)
        return result
...
indices = torch.randint(0, 50, (128, 25), dtype=torch.int64, device=DEVICE)
y = torch.randn(128, 25, device=DEVICE)
