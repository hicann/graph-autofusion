...
const std::vector<uint32_t> kMatMulInputInstanceNum = {1U, 1U, 0U, 0U};
...
struct RuntimeTilingKey {
  ...
  bool operator<(const RuntimeTilingKey &other) const {
    return std::tie(soc_version, device_id, op_type, dtype, format, input0_shape, input1_shape, output_shape, is_batch,
                    transpose_x1, transpose_x2, offset_x, op_impl_mode, enable_hf32, aicore_num, aiv_num) <
           std::tie(other.soc_version, other.device_id, other.op_type, other.dtype, other.format, other.input0_shape,
                    other.input1_shape, other.output_shape, other.is_batch, other.transpose_x1, other.transpose_x2,
                    other.offset_x, other.op_impl_mode, other.enable_hf32, other.aicore_num, other.aiv_num);
  }
};
...
struct CompileStateKey {
  ...
  bool operator<(const CompileStateKey &other) const {
    return std::tie(soc_version, device_id, dtype, format, is_batch, transpose_x1, transpose_x2, offset_x, op_impl_mode,
                    enable_hf32, aicore_num, aiv_num) < std::tie(other.soc_version, other.device_id, other.dtype,
                                                                 other.format, other.is_batch, other.transpose_x1,
                                                                 other.transpose_x2, other.offset_x, other.op_impl_mode,
                                                                 other.enable_hf32, other.aicore_num, other.aiv_num);
  }
};
...
std::unique_ptr<CompileState> BuildCompileState(const CompileInfo &compile_info, const OpHostSchema &schema,
                                                ...
                                                ge::Format format, const MatMulAttrs &attrs, std::string &error_msg) {
  ...
  auto parse_holder = parse_builder.OpType(schema.op_type)
                          ...
                          .IOInstanceNum(kMatMulInputInstanceNum, kSingleOutputInstanceNum)
                          .InputTensorDesc(0, data_type, format, format)
                          .InputTensorDesc(1, data_type, format, format)
                          .InputTensorDesc(2, data_type, format, format)
                          .InputTensorDesc(3, ge::DT_INT8, format, format)
...
CompileStateKey MakeCompileStateKey(const CompileInfo &compile_info, bool is_batch, const std::string &dtype,
...
std::shared_ptr<const CompileState> GetCompileState(const CompileInfo &compile_info, const OpHostSchema &schema,
                                                    ...
                                                    std::string &error_msg) {
  const CompileStateKey key = MakeCompileStateKey(compile_info, is_batch, dtype, format_name, attrs);
  {
  ...
  auto state = BuildCompileState(compile_info, schema, funcs, is_batch, data_type, format, attrs, error_msg);
  if (state == nullptr) {
    return nullptr;
  }
...
TilingRequest MakeTilingRequest(const CompileInfo &compile_info, const std::vector<TensorInfo> &inputs,
  ...
  return TilingRequest{compile_info, inputs, outputs, is_batch, schema, ReadMatMulAttrs(attrs, is_batch),
...
bool IsSupportedTilingTensorDesc(const std::vector<TensorInfo> &inputs, const std::vector<TensorInfo> &outputs,
  ...
  return data_type != ge::DT_UNDEFINED && format != ge::FORMAT_RESERVED &&
         DtypeToGeDataType(inputs[1].dtype) == data_type && DtypeToGeDataType(outputs[0].dtype) == data_type &&
         FormatToGeFormat(inputs[1].format) == format && FormatToGeFormat(outputs[0].format) == format;
}
...
bool RunSharedCubeTiling(const TilingRequest &request, TilingResult &result) {
  ...
  auto compile_state = GetCompileState(request.compile_info, request.schema, funcs, request.is_batch, request.data_type,
                                       ...
                                       request.inputs[0].format, result.error_msg);
  ...
  std::array<gert::Tensor, 2> input_tensors_storage = {
      gert::Tensor(MakeStorageShape(request.inputs[0]), storage_format, request.data_type),
      gert::Tensor(MakeStorageShape(request.inputs[1]), storage_format, request.data_type)};
  ...
  scratch.input_tensors.push_back(&input_tensors_storage[0]);
  scratch.input_tensors.push_back(&input_tensors_storage[1]);
