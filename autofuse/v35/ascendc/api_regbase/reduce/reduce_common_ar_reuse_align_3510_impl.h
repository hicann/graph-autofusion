...
#ifndef AUTOFUSE_REDUCE_EXTEND_REDUCE_COMMON_AR_REUSE_ALIGN_C310_IMPL_H
#define AUTOFUSE_REDUCE_EXTEND_REDUCE_COMMON_AR_REUSE_ALIGN_C310_IMPL_H
...
__simd_callee__ inline void ReduceARfoldOneToThree(Reg::RegTensor<T, Trait> &vreg0, Reg::RegTensor<T, Trait> &vreg2,
                                                   Reg::MaskReg &fullMask) {
  if constexpr (IsSameType<T, bfloat16_t>::value) {
    ...
    ReduceOpInternal::ReduceARCastfoldOneToThree<T, half, Trait, ReduceOpInternal::CastTraitB8F16,
                                                 ReduceOpInternal::CastTraitF16B8, Binaryfunc, Reducefunc>(vreg0, vreg2,
                                                                                                           fullMask);
  } else {
...
template <class T, const Reg::RegTrait &Trait, const uint16_t vlSize, auto Binaryfunc, auto Reducefunc,
          bool isReuseSource>
__simd_vf__ inline void ReduceARB64OverVLVF(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr,
                                            uint32_t dimA, uint32_t dimR, uint32_t mainR, uint32_t tailR,
                                            uint16_t dimRAxis, uint16_t inplaceRepeats, uint32_t inplaceTail,
                                            uint16_t needInplaceAdd, uint16_t copyRepeats, uint16_t base,
                                            uint16_t folds, uint16_t avgFolds, uint16_t mainTimes, uint16_t foldZero,
...
template <class T, const Reg::RegTrait &Trait, const uint16_t vlSize, auto Binaryfunc, auto Reducefunc,
          bool isReuseSource>
__simd_vf__ inline void ReduceAROverVLVFImpl(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr,
                                             uint32_t dimA, uint32_t dimR, uint32_t mainR, uint32_t tailR,
                                             uint16_t dimRAxis, uint16_t inplaceRepeats, uint32_t inplaceTail,
                                             uint16_t needInplaceAdd, uint16_t copyRepeats, uint16_t base,
                                             uint16_t folds, uint16_t avgFolds, uint16_t mainTimes, uint16_t foldZero,
  ...
  for (uint16_t i = 0; i < foldZero; i++) {
    if constexpr (IsSameType<T, bfloat16_t>::value) {
      ReduceARCastfoldZero<T, float, Trait, ReduceOpInternal::CastTraitBF16F32, ReduceOpInternal::CastTraitF32BF16,
                           vlSize, Binaryfunc, Reducefunc>(dstAddr, addr, dimA, dimR, fullMask);
    } else if constexpr (SupportBytes<T, 1>()) {
      ReduceARCastfoldZero<T, half, Trait, ReduceOpInternal::CastTraitB8F16, ReduceOpInternal::CastTraitF16B8, vlSize,
...
template <class T, const Reg::RegTrait &Trait, const uint16_t vlSize, auto Binaryfunc, auto Reducefunc,
          bool isReuseSource>
__aicore__ inline void ReduceAROverVLImpl(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr, uint32_t dimA,
                                          uint32_t dimR) {
  uint32_t mainR = ReduceOpInternal::CalculateMainR(dimR, true, vlSize);
  uint32_t tailR = dimR - mainR;
  // A is at least 8 Bytes, dimRAxis will not over b16 max
...
template <class T, const Reg::RegTrait &Trait, auto Binaryfunc, auto Reducefunc, bool isReuseSource = false,
          ReduceType groupReduceType = ReduceType::NONE>
__aicore__ inline void ReduceARImpl(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr, uint32_t dimA,
                                    uint32_t dimR) {
  constexpr uint16_t vlSize = SupportBytes<T, 8>() ? GetVecLen() / sizeof(float) : GetVecLen() / sizeof(T);
  if (dimR <= vlSize) {
    ReduceARReuseSourceLessThanVL<T, Trait, vlSize, Binaryfunc, Reducefunc, groupReduceType>(dstAddr, srcAddr, dimA,
                                                                                             dimR);
  } else {
    if constexpr (SupportBytes<T, 8>()) {
      ReduceARB64OverVL<T, Trait, vlSize, Binaryfunc, Reducefunc, isReuseSource>(dstAddr, srcAddr, tmpAddr, dimA, dimR);
    } else {
      ReduceAROverVLImpl<T, Trait, vlSize, Binaryfunc, Reducefunc, isReuseSource>(dstAddr, srcAddr, tmpAddr, dimA,
                                                                                  dimR);
    }
  }
}
...
#endif  // AUTOFUSE_REDUCE_EXTEND_REDUCE_COMMON_AR_REUSE_ALIGN_C310_IMPL_H
