...
#ifndef AUTOFUSE_REDUCE_EXTEND_REDUCE_COMMON_AR_REUSE_ALIGN_LESS_THAN_VL_C310_IMPL_H
#define AUTOFUSE_REDUCE_EXTEND_REDUCE_COMMON_AR_REUSE_ALIGN_LESS_THAN_VL_C310_IMPL_H
...
__simd_callee__ inline void GroupReduce(Reg::RegTensor<T, Trait> &dst, Reg::RegTensor<T, Trait> &src,
                                        Reg::MaskReg &mask) {
...
template <class T, const Reg::RegTrait &Trait, const uint16_t vlSize, auto Binaryfunc, auto GroupReduceType,
          bool NeedFoldR = true>
__simd_vf__ inline void GroupReduceVf(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, uint16_t innerFoldNum, uint16_t fusedA,
                                      uint16_t strideA, uint32_t outerRepElement) {
...
__aicore__ inline void GroupReduceARLessThanVL(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, uint32_t dimA, uint32_t dimR) {
  ...
  GroupReduceVf<T, Trait, vlSize, Binaryfunc, groupReduceType, NeedFoldR>(dstAddr, srcAddr, innerFoldNum, fusedA,
                                                                          strideA, outerRepElement);
}
...
__aicore__ inline void ReduceARReuseSourceLessThanVL(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, uint32_t dimA,
  ...
  if (dimR == 1) {
    ...
    ReduceARCastLessThanVL<T, half, Trait, ReduceOpInternal::CastTraitB8F16, ReduceOpInternal::CastTraitF16B8, vlSize,
...
#endif  // AUTOFUSE_REDUCE_EXTEND_REDUCE_COMMON_AR_REUSE_ALIGN_LESS_THAN_VL_C310_IMPL_H
