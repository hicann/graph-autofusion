...
#ifndef AUTOFUSE_REDUCE_EXTEND_REDUCE_MIN_REDUCE_MIN_C310_IMPL_H_
#define AUTOFUSE_REDUCE_EXTEND_REDUCE_MIN_REDUCE_MIN_C310_IMPL_H_
...
__aicore__ inline void ReduceMinARB64ReuseSourceCompute(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr,
...
template <class T, bool isReuseSource = false>
__aicore__ inline void ReduceMinARReuseSourceCompute(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr,
                                                     const uint32_t srcShape[]) {
  using castType = typename ReduceOpInternal::ExtractReduceCastType<T>::CastT;
  if ((srcShape[1] * sizeof(T)) % 32 == 0 || srcShape[1] == 1) {
    ReduceARImpl<T, Reg::RegTraitNumOne, Reg::Min<T, Reg::MaskMergeMode::ZEROING, Reg::RegTensor<T>>,
    ...
    if constexpr (SupportBytes<T, 1>()) {
      ...
      ReduceARReuseSourceUnAligned<T, Reg::RegTraitNumOne, Reg::Min<T, Reg::MaskMergeMode::ZEROING, Reg::RegTensor<T>>,
                                   Reg::ReduceMin<castType, Reg::MaskMergeMode::ZEROING, Reg::RegTensor<castType>>>(
          dstAddr, srcAddr, srcShape[0], srcShape[1]);
    }
  }
...
template <class T, bool isReuseSource = false>
__aicore__ inline void ReduceMinRAB64ReuseSourceCompute(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr,
                                                        const uint32_t srcShape[]) {
  if ((srcShape[1] * sizeof(T)) % 32 == 0) {
    if (srcShape[1] * sizeof(T) <= GetVecLen() / 2) {
      ReduceRAConcatImpl<T, Reg::RegTraitNumTwo,
                         Reg::Min<T, Reg::MaskMergeMode::ZEROING, Reg::RegTensor<T, Reg::RegTraitNumTwo>>,
...
template <class T, bool isReuseSource = false>
__aicore__ inline void ReduceMinRAReuseSourceCompute(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr,
                                                     const uint32_t srcShape[]) {
  if ((srcShape[1] * sizeof(T)) % 32 == 0) {
    ReduceRAImpl<T, Reg::RegTraitNumOne, Reg::Min<T, Reg::MaskMergeMode::ZEROING, Reg::RegTensor<T>>, isReuseSource>(
        dstAddr, srcAddr, tmpAddr, srcShape[1], srcShape[0]);
  } else {
    ReduceRAReuseSourceUnAligned<T, Reg::RegTraitNumOne, Reg::Min<T, Reg::MaskMergeMode::ZEROING, Reg::RegTensor<T>>>(
        dstAddr, srcAddr, srcShape[1], srcShape[0]);
  }
}

template <class T, class pattern, bool isReuseSource = false>
__aicore__ inline void ReduceMinImpl(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  static_assert(
      SupportType<T, int8_t, uint8_t, int16_t, uint16_t, half, bfloat16_t, int32_t, uint32_t, float, int64_t,
                  uint64_t>(),
...
#endif  // AUTOFUSE_REDUCE_EXTEND_REDUCE_MIN_REDUCE_MIN_C310_IMPL_H_
