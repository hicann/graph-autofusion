...
#ifndef AUTOFUSE_REDUCE_EXTEND_MEAN_C310_IMPL_H
#define AUTOFUSE_REDUCE_EXTEND_MEAN_C310_IMPL_H

#include "kernel_tensor.h"
#include "kernel_basic_intf.h"
#include "adv_api/reduce/mean_utils.h"
...
__simd_callee__ inline void LoadSrcData(Reg::RegTensor<ConvType> &srcReg, __ubuf__ T *src, uint16_t index,
                                        uint32_t offset, Reg::MaskReg &mask) {
  ...
  if constexpr (std::is_same<T, half>::value && std::is_same<ConvType, float>::value) {
    ...
    Reg::Cast<float, T, AscendC::Internal::castTraitB16ToB32>(srcReg, srcTmpReg, mask);
  } else {
...
__simd_vf__ inline void MeanForOneRepeatTime(__ubuf__ T *dstUb, __ubuf__ U *srcUb, const MeanParams meanParams,
                                             uint32_t calCount, uint32_t offset) {
  uint32_t count;
  ...
  for (int i = 0; i < meanParams.outter; i++) {
    ...
    if constexpr (sizeof(T) == sizeof(half) && sizeof(ConvType) == sizeof(float)) {
      Reg::Cast<T, float, AscendC::Internal::castTraitB32ToB16>(dstReg, dstTmpReg, mask);
...
__aicore__ inline void MeanImpl(const LocalTensor<T> &dstTensor, const LocalTensor<T> &srcTensor,
  ...
  MeanCheckParams<T, accType, isReuseSource, isBasicBlock, reduceDim>(dstTensor, srcTensor, sharedTmpBuffer,
                                                                      meanParams);
  ...
  while (totalCnt != 0) {
    ...
    if (totalCnt == 1) {
      Internal::MeanForOneRepeatTime<T, ConvType, ConvType>(dstUb, sharedTmpBufferUb, meanParams, calCount, offset);
    } else {
      Internal::ReduceSumNextN<ConvType, ConvType, false>(sharedTmpBufferUb, sharedTmpBufferUb, meanParams, calCount,
                                                          loopRepeatTimes, offset);
    }
    --totalCnt;
  }
}
...
#endif  // AUTOFUSE_REDUCE_EXTEND_MEAN_C310_IMPL_H
