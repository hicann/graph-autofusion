...
#ifndef AUTOFUSE_REDUCE_EXTEND_SUM_C310_IMPL_H
#define AUTOFUSE_REDUCE_EXTEND_SUM_C310_IMPL_H

#include "kernel_basic_intf.h"
#include "kernel_tensor.h"
#include "adv_api/reduce/sum_utils.h"

namespace AscendC {
namespace ReduceExtendInternal {
namespace SumInternal {
template <typename T>
__simd_vf__ inline void SumForOneRepeatTime(__ubuf__ T *dstUb, __ubuf__ T *srcUb, const SumParams sumParams,
...
__aicore__ inline void SumCompute(const LocalTensor<T> &dstTensor, const LocalTensor<T> &srcTensor,
  ...
  while (totalCnt != 0) {
    ...
    if (totalCnt == 1) {
      ...
      SumInternal::ReduceSumNextN<T, false>(sharedTmpBufferUb, sharedTmpBufferUb, sumParams, calCount, loopRepeatTimes,
                                            offset);
    }
    --totalCnt;
  }
...
#endif  // AUTOFUSE_REDUCE_EXTEND_SUM_C310_IMPL_H
