...
__aicore__ inline void ChebyshevPolynomialUCal(__ubuf__ T *dst, __ubuf__ T *src, uint32_t calCount) {
...
template <typename T, int64_t N>
__aicore__ inline void ChebyshevPolynomialUExtend(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  if constexpr (N < 0) {
    ...
    VF_CALL<ChebyshevPolynomialUCal<T, N>>((__ubuf__ T *)dst.GetPhyAddr(), (__ubuf__ T *)src.GetPhyAddr(), calCount);
  }
}
