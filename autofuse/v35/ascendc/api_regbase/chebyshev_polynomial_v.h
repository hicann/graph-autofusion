...
#ifndef __ASCENDC_API_REGBASE_CHEBYSHEV_POLYNOMIAL_V_H__
#define __ASCENDC_API_REGBASE_CHEBYSHEV_POLYNOMIAL_V_H__
...
__aicore__ inline void ChebyshevPolynomialVCal(__ubuf__ T *dst, __ubuf__ T *src, uint32_t calCount) {
...
template <typename T, int64_t N>
__aicore__ inline void ChebyshevPolynomialVExtend(const LocalTensor<T> &dst, const LocalTensor<T> &src,
  ...
  if constexpr (N < 0) {
    ...
    VF_CALL<ChebyshevPolynomialVCal<T, N>>((__ubuf__ T *)dst.GetPhyAddr(), (__ubuf__ T *)src.GetPhyAddr(), calCount);
  }
}
