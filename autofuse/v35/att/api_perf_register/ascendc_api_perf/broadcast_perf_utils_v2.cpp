...
af::Status GetBroadcastVectorElements(const std::string &dtype, Expr &vl, Expr &half_vl, Expr &block_elements) {
  Expr byte_size;
  GE_ASSERT_SUCCESS(GetDtypeByteSize(dtype, byte_size));
  GE_ASSERT_TRUE(af::SymbolicUtils::StaticCheckEq(byte_size, CreateExpr(0)) != af::TriBool::kTrue,
                 "Broadcast dtype byte size cannot be zero.");
  vl = CreateExpr(kVectorBytes) / byte_size;
  GE_ASSERT_TRUE(2U != 0U, "Broadcast half vector divisor cannot be zero.");
  half_vl = vl / kSymTwo;
  GE_ASSERT_TRUE(af::SymbolicUtils::StaticCheckEq(byte_size, CreateExpr(0)) != af::TriBool::kTrue,
                 "Broadcast dtype byte size cannot be zero.");
...
af::Status CeilDiv(const Expr &value, const Expr &divisor, Expr &result) {
  GE_ASSERT_TRUE(af::SymbolicUtils::StaticCheckEq(divisor, CreateExpr(0)) != af::TriBool::kTrue,
                 "Broadcast ceil divisor cannot be zero.");
